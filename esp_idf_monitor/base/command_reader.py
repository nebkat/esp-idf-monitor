...
class CommandReader(StoppableThread):
    ...
    def _expect(self, pattern):
        ...
        """Block until a serial line matching the pattern is observed, while
        staying responsive to stop(). Output that was already received before
        this command was reached is matched as well: the look-back buffer is
        scanned first (consuming up to and including the match), and only if the
        pattern is not there yet is it armed to wait for new lines. Serial output
        continues to be processed and printed by the main loop in the meantime."""
        with self._expect_lock:
